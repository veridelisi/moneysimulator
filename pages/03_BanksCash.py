...
SCENARIOS = [
    {
        ...
        "short": "Bank X creates a loan asset and a matching deposit liability for Customer A.",
        "insight": "Bank X does not lend out pre-existing reserves or cash. It expands both sides of its balance sheet: <strong>Loan +</strong> on the asset side and <strong>Customer A Deposit +</strong> on the liability side. Customer A receives purchasing power as a bank deposit.",
        "tag": "💚 Deposit Money Created", "tag_type": "green",
        "choice_type": "loan",
        "training_amt": 200,
    ...
    {
        "id": 2, "emoji": "🏛️",
        "title": "Central Bank Provides Reserves to Bank X",
        "short": "The Central Bank lends reserves to Bank X, creating reserve balances on the Central Bank ledger.",
        "insight": "Reserves are money used between banks and the Central Bank. They are not customer deposits and do not enter M1 directly. Here, Bank X gains a reserve asset, while the Central Bank records a loan to Bank X and a reserve liability.",
        "tag": "➡️ No M1 Change", "tag_type": "blue",
        "choice_type": "reserve",
        "training_amt": 200,
        "sim_opts": [100, 200, 300, 400],
        "sim_label": "How much reserve funding does the Central Bank provide to Bank X?",
        "involved": ["CentralBank", "BankX"],
    },
    {
        "id": 3, "emoji": "💵",
        "title": "Bank X Gets Cash from the Central Bank",
        "short": "Bank X converts part of its reserves into vault cash.",
        "insight": "This is the key reserve-drain step. Bank X gives up reserves at the Central Bank and receives physical cash for its vault. Bank X's asset mix changes: <strong>Reserves −</strong> and <strong>Cash +</strong>. The public still has no cash yet, so M1 does not change at this step.",
        "tag": "🔀 Reserve Drain into Vault Cash", "tag_type": "blue",
        "choice_type": "cash_bank",
        "training_amt": 80,
        "sim_opts": [40, 80, 120, 160],
        "sim_label": "How much cash does Bank X withdraw from the Central Bank?",
        "involved": ["CentralBank", "BankX"],
    },
    {
        "id": 4, "emoji": "🏧",
        "title": "Customer A Withdraws Cash from Bank X",
        "short": "Customer A converts part of the deposit into physical cash.",
        "insight": "Now the customer deposit shrinks and currency enters the hands of the public. Bank X loses vault cash and its deposit liability to Customer A falls. Customer A's asset changes form: <strong>Deposits −</strong> and <strong>Cash +</strong>. M1 usually stays unchanged because deposit money is replaced by currency in circulation.",
        "tag": "🔀 Deposit Becomes Currency", "tag_type": "blue",
        "choice_type": "cash_cust",
        "training_amt": 50,
        "sim_opts": [20, 50, 70, 80],
        "sim_label": "How much cash does Customer A withdraw from Bank X?",
        "involved": ["BankX", "CustomerA"],
    },
    {
        "id": 5, "emoji": "🎓",
        "title": "Withdrawal Mechanics and Reserve Drain Review",
        "short": "Review the full path: loan creates deposit, reserves support cash conversion, and cash withdrawal drains vault cash.",
        "insight": "The full chain is: <strong>Bank loan creates a deposit</strong>; <strong>Central Bank reserves fund Bank X</strong>; <strong>Bank X converts reserves into vault cash</strong>; then <strong>Customer A converts deposits into currency</strong>. The important lesson: lending creates deposits, but cash withdrawals require banks to obtain currency, which drains reserves when supplied by the Central Bank.",
        "tag": "🎓 Complete!", "tag_type": "green",
        "choice_type": "none",
        "training_amt": 0,
        "sim_opts": [],
        "sim_label": "",
        "involved": ["CentralBank", "BankX", "CustomerA"],
    },
]
...
ENTITY_ORDER = ["CentralBank","BankX","CustomerA"]
FRIENDLY = {
    "CustADep":"Cust A Dep",
    "DueCB":"Due to CB",
    "LoansToBanks":"Loans→Bank X",
}
...
def build_transactions(sc_id, amt):
    ...
    # Credit decreases assets and increases liabilities.
    if sc_id == 1:
        # Bank X loan creates Customer A deposit.
        return [
        ...
        # Central Bank lends reserves to Bank X.
        return [
            ("BankX","debit","Reserves",amt),
            ("BankX","credit","DueCB",amt),
            ("CentralBank","debit","LoansToBanks",amt),
            ("CentralBank","credit","Reserves",amt),
        ]
    elif sc_id == 3:
        # Bank X converts reserves into vault cash.
        return [
            ("BankX","debit","Cash",amt),
            ("BankX","credit","Reserves",amt),
            ("CentralBank","debit","Reserves",amt),
            ("CentralBank","credit","Cash",amt),
        ]
    elif sc_id == 4:
        # Customer A withdraws cash from Bank X.
        return [
            ("BankX","debit","CustADep",amt),
            ("BankX","credit","Cash",amt),
            ("CustomerA","debit","Cash",amt),
...
def build_flow(sc_id, amt):
    ...
    if sc_id == 1:
        ...
        return [CB, arr(f"{a} reserves","to Bank X"), BX]
    elif sc_id == 3:
        return [CB, arr(f"{a} cash","reserves drained"), BX]
    elif sc_id == 4:
        return [BX, arr(f"{a} cash","deposit converted"), CA]
    return []
...
if step_i >= len(SCENARIOS):
    st.markdown(
        ...
        'You traced withdrawal mechanics from deposit creation to reserve drain and customer cash withdrawal. All accounted for.'
        '</div></div>',
        unsafe_allow_html=True
    )
    bm, cm, tot = compute_ms(st.session_state[ss("ledger")])
